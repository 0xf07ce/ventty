//
// ventty Terminal — interactive demo
// (adapted from sample1/src/main.cpp)
//
...
int main()
{
    ...
    infoWin->drawText(2, 0, " ventty Terminal Engine ", yellowBold);
    ...
    term.onKey([&](KeyEvent const & ev)
    {
        ...
        bool moved = false;
        if (ev.key == KeyEvent::Key::Up)    { boxY--; moved = true; }
        if (ev.key == KeyEvent::Key::Down)  { boxY++; moved = true; }
        if (ev.key == KeyEvent::Key::Left)  { boxX--; moved = true; }
        if (ev.key == KeyEvent::Key::Right) { boxX++; moved = true; }
    ...
    while (term.isRunning())
    {
        ...
        // Draw status line at bottom of root screen
        std::string status = " ventty | " +
