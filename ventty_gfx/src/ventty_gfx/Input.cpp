...
{
void Input::setCellSize(int cellW, int cellH)
{
    _cellWidth = cellW;
    _cellHeight = cellH;
}

void Input::setScale(int scale)
{
    _scale = scale > 0 ? scale : 1;
}

std::pair<int, int> Input::pixelToCell(float px, float py) const
{
...
bool Input::processEvent(SDL_Event const & event)
{
    switch (event.type)
    {
    ...
    case SDL_EVENT_TEXT_INPUT:
        {
            ...
            while (pos < text.size())
            {
                auto cp = ventty::utf8::decode(text, pos);
                if (cp == U'\0')
                    break;
