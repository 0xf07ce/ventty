...
bool GfxRenderer::init(int cols, int rows, int cellW, int cellH, std::string const & title, int scale)
{
    _cellW = cellW;
    _cellH = cellH;
    _fbWidth = cols * cellW;
...
void GfxRenderer::renderCell(ventty::Cell const & cell, int px, int py,
    GfxFont & font, int cellW, int cellH)
{
    ...
    GlyphInfo const * glyph = font.getGlyph(cell.ch);
    if (!glyph)
        return;

    SDL_Texture * tex = font.getTexture(glyph->fontIndex);
    if (!tex)
        return;
    ...
    SDL_FRect srcRect = {
        static_cast<float>(glyph->atlasX),
    ...
    SDL_RenderTexture(_renderer, tex, &srcRect, &dstRect);
...
void GfxRenderer::renderWindow(ventty::Window & win, GfxFont & font)
{
...
SDL_Window * GfxRenderer::sdlWindow() const { return _sdlWindow; }
SDL_Renderer * GfxRenderer::sdlRenderer() const { return _renderer; }
int GfxRenderer::scale() const { return _scale; }
} // namespace ventty
