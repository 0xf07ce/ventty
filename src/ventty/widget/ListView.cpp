...
std::vector<ListItem> const & ListView::items() const
{
    return _items;
}
...
int ListView::scrollOffset() const
{
    return _scrollOffset;
}

Style const & ListView::normalStyle() const
{
    return _normalStyle;
}

Style const & ListView::selectedStyle() const
{
    return _selectedStyle;
}

void ListView::setNormalStyle(Style const & s)
{
    _normalStyle = s;
}

void ListView::setSelectedStyle(Style const & s)
{
    _selectedStyle = s;
}

void ListView::setOnSelectionChanged(SelectionCallback cb)
{
    _onSelectionChanged = std::move(cb);
}

void ListView::setOnActivate(ActivateCallback cb)
{
    _onActivate = std::move(cb);
}
...
void ListView::draw(Window & window)
{
    ...
    for (int i = 0; i < visible; ++i)
    {
        ...
        if (itemIndex < static_cast<int>(_items.size()))
        {
            ...
            for (size_t j = 0; j < item.text.size() && col < _rect.width;)
            {
                ...
                window.drawText(_rect.x + col, y, item.text.substr(j, charBytes), style);
                col += charWidth;
                j += charBytes;
            }
            ...
            {
                window.putChar(_rect.x + col, y, U' ', style);
                ++col;
            }
        }
        else
        {
            ...
            {
                window.putChar(_rect.x + j, y, U' ', _normalStyle);
            }
        }
    }
}
